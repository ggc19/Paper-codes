...
print('TCNN test loss:', scores[0])
print('TCNN test accuracy:', scores[1])
from keras.utils import plot_model
plot_model(model, to_file='TCNN model.png',show_shapes=True)
...
print('TCNN accuracy：',(time_con_matrix[0,0]+time_con_matrix[1,1])/len(y_pred))
time_fpr, time_tpr, time_thresholds = roc_curve(y_test[:,0], y_prob)
time_roc_auc = auc(time_fpr, time_tpr)
plt.figure()
plt.plot(time_fpr, time_tpr,label='TCNN (AUC = %0.2f)' % (time_roc_auc))
plt.title('Roc Curve')
plt.legend(loc='best')
plt.show()
